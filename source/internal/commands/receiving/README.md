...
## Why we use it?
"What is it doing" and "what is it listening to" are different questions, and only the first had an answer. `status` reports the connection, the firmware and the mode, all of which stay true for hours. `scanning` reports the channels in the rotation, which means moving the radio and takes seconds. Neither says what is coming out of the speaker right now, which is the reading anything following the radio actually needs: it is what labels a recording, and it is what a person or an agent asks the moment the scanner stops on something.

The command exists to make one distinction, and everything about its shape follows from it. A scanning radio names a channel too. Asked while it is stepping through its lists, it answers with whatever channel it happened to be checking at that instant, and that reply is indistinguishable from one about a transmission unless something says which it is. So `receiving` is the first field, it is the one to read before any other, and when it is no the command says on stderr that the channel was only passed rather than stopped on. Reporting the channel and leaving the reader to work the rest out is how this goes wrong.
