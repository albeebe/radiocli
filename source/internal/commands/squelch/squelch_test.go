...
func TestNew(t *testing.T) {
	...
	t.Run("Wiring", func(t *testing.T) {
		...
		if cmd.Use != "squelch" {
			t.Errorf("the command is %q, wanted %q", cmd.Use, "squelch")
		}
	...
	t.Run("Runs", func(t *testing.T) {
		...
		if got, want := out.String(), "squelch: 2 of 15\n"; got != want {
...
func Test_newSet(t *testing.T) {
	...
	t.Run("Sets", func(t *testing.T) {
		...
		if sent != "SQL,5" {
			t.Errorf("the scanner was sent %q, wanted %q", sent, "SQL,5")
		}
		if got, want := out.String(), "squelch: 5 of 15\n"; got != want {
	...
	t.Run("RefusesTypo", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "invalid squelch level") {
...
func Test_renderLevel(t *testing.T) {
	...
	t.Run("Text", func(t *testing.T) {
		...
		if err := renderLevel(app, 3); err != nil {
			t.Fatalf("renderLevel: %v", err)
		}
		if got, want := out.String(), "squelch: 3 of 15\n"; got != want {
	...
	t.Run("JSON", func(t *testing.T) {
		...
		want := report{Level: 3, Min: device.MinLevel, Max: device.MaxLevel}
		if got != want {
...
func Test_runGet(t *testing.T) {
	...
	t.Run("Reads", func(t *testing.T) {
		...
		if got, want := out.String(), "squelch: 9 of 15\n"; got != want {
	...
	t.Run("ReadFails", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "reading the squelch") {
			t.Errorf("runGet reported %q, wanted it to name reading the squelch", err)
		}
	})
}
...
func Test_runSet(t *testing.T) {
	...
	t.Run("Sets", func(t *testing.T) {
		...
		if err := runSet(context.Background(), app, 4); err != nil {
			t.Fatalf("runSet: %v", err)
		}
		if got, want := out.String(), "squelch: 4 of 15\n"; got != want {
	...
	t.Run("Differs", func(t *testing.T) {
		...
		if !strings.Contains(notes.String(), "rather than the 4") {
	...
	t.Run("NoDevice", func(t *testing.T) {
		...
		err := runSet(context.Background(), app, 4)
		if !errors.Is(err, appcontext.ErrNoDevice) {
	...
	t.Run("SetFails", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "setting the squelch") {
			t.Errorf("runSet reported %q, wanted it to name setting the squelch", err)
		}
	})
	...
	t.Run("ReadBackFails", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "reading the squelch back") {
			t.Errorf("runSet reported %q, wanted it to name reading the squelch back", err)
		}
	})
}
