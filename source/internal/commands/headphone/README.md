...
## Why we use it?
The jack on this scanner is wired out of phase, and the setting exists to correct it. Uniden addressed the fault in firmware rather than in hardware, by adding a menu to flip one side, which means whether a given radio has the problem depends entirely on which way its owner has left that menu. There was no way to find out without picking the radio up and walking the menus by hand.

What makes it worth a command of its own is that the symptom does not look like a phase problem. Anything that combines the two sides of the jack into one signal, which is what taking mono off a stereo input means, gets audio around eleven decibels too quiet with the body taken out of the voice. The low frequencies are the most alike between the two sides and cancel the most completely, so what survives is thin and reedy: it sounds like the speaker is talking through a kazoo. Measured on an SDS150, folding the two sides moved the energy in a voice from 67% below 1 kHz to 13%. Nothing about it says "phase", and the natural places to lay the blame are the radio, the cable, or a digital voice codec, none of which are at fault. The measurements are written up in [research/oddities.md](../../../../research/oddities.md).

Recording does not depend on this. `audio record` measures what folding the two sides actually produces and takes a single side when folding would destroy the signal, so it copes with either setting without being told which one the radio is on, and says so when it finds the problem. That is deliberate: the setting is not the whole answer, because a cable or an adapter can invert a side just as easily as the firmware can, and only measuring the signal that actually arrives accounts for both. This command is for fixing it at the source instead, which is better where it is possible, because it is then right for everything the radio is ever plugged into rather than for this tool alone.

The setting is not in the scanner's remote protocol. It lives in a menu, so even reading it walks into Settings and back out, which stops the scan for a moment. That is why nothing about it happens automatically, and why this command does not carry `appcontext.OnlyReads`.
