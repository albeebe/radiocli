...
`audio listen` is the shortest of the three verbs and the one most people want: it plays the radio on this computer, and writes nothing anywhere. It squelches by default, because between transmissions a scanner's output is hiss and an evening of that is not worth listening to, and the detector it squelches with is `audiogate`, the same one that finds transmissions for the recorder.

Which question it asks that detector took two attempts to get right, and both wrong answers sound the same from the speaker: holes in audio that is perfect in the recording of it. The gate holds finished audio back for the length of its hang, because that is how the end of a recording gets trimmed, so playing what it hands back puts every word a hang behind the radio. It also says nothing about a transmission at all until one has outlived `--min-duration` and earned a file, so opening the speakers on that loses the first half second of every transmission, which in dispatch traffic is most of the first word each time. Measured against the defaults, that second one was 480 ms gone, every time somebody keyed up.

So the gate grew `Live`, which is the question a speaker asks rather than the question a file asks: is this frame, the one that has just arrived, part of a transmission as far as anybody listening now is concerned. It is true from the first frame above the noise floor and stays true while a transmission is open, hang included. `audio record --listen` is the same idea from the other end, a recorder that also plays what it is writing, and it asks exactly the same question. The consequence is worth stating plainly: a blip too short to be worth a file is heard and not kept, which is what a scanner's own speaker does with one.
