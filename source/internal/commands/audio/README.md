...
## Further reading
- **Cobra commands** - The command is a `*cobra.Command` built by `New`, and `audio output` is a subcommand attached to it, which is how every command in this tool is wired into the tree
