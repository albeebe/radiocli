...
Keeping this in its own package means every command reads the scanner's database the same way, through one set of functions, instead of each one growing its own parser and its own idea of what a department is. It also gives the tool a single place for the two rules that are easy to get wrong and expensive to relearn: that a name has to be resolved into an index by searching, because an index carries no hint of what holds it, and that the built-in scan sources must never be walked, because asking the full RadioReference database for its systems floods the serial port and corrupts every exchange that follows.
...
// Walk down into the systems that list holds.
