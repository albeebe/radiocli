...
## What this does?
This package holds the three formatters every command's output uses: `JSON` writes a value as the indented JSON that `--output json` produces, `Dash` turns an empty value into `-` so a blank column in a table cannot be mistaken for a column that ran out of rows, and `YesNo` writes a boolean the way a listing reads it. That is the whole package. Nothing here knows what a command is, what the scanner is, or what is being printed.
